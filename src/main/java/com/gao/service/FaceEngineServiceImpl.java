...
import com.gao.repository.UserDao;
import com.gao.util.Base64Util;
...
//                    log.info("年龄：{}",ageInfoList.get(0).getAge());
                    /**
                    * 报错原因：未在config中配置可以检测age
                     * 解决方法：
                     * 1. 直接添加配置(有点low)
                     * 2. 使用spring框架的加载配置文件（有点吊）
                     */
    ...
    public User createNewUser(byte[] detectFaces) {
        ...
        System.out.println("detectFaces: "+detectFaces);
